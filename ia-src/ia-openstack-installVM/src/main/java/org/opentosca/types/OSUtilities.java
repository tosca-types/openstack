...
{	
	public static OSClient Authorize(JSONObject credentials, JSONObject endpointsAPI)
	{
	...
	public static String CreateServer(OSClient os, String server_name, String image_id, String flavor_id, String keypair) throws ClientResponseException, Exception
	{
		Server server = null;
		String server_id = "";
		...
		String retParams = "";
				// Create a Server Model Object
					...
					ServerCreate sc = Builders.server().name(server_name).flavor(flavor_id).image(image_id).keypairName(keypair).build();
					// Boot the Server
					server = os.compute().servers().boot(sc);
					String servers[] = server.toString().split(",");
					server_id = servers[0].split("=")[1]; 									 
					
					java.util.List<? extends FloatingIP> ips = os.compute().floatingIps().list();					
					...
					{
							FloatingIP floating_ip = os.compute().floatingIps().allocateIP("nova-informatik-intern");
		...
		ActionResponse r = os.compute().floatingIps().addFloatingIP(server,floatingIpAddress);
		retParams = server_id + ";" + floatingIpAddress;		
								
		return retParams;
	}
	
	public static void StopServer(OSClient os, String server_id) throws InterruptedException
	{
	...
	public static void StartServer(OSClient os, String server_id) throws InterruptedException
	{
		Status status = os.compute().servers().get(server_id).getStatus();
		System.out.print(status);
		if (!status.toString().contentEquals("ACTIVE"))
	...
	public static String CreateKeypair(OSClient os, String keypair) throws InterruptedException
	{		
	...
	public static void TerminateServer(OSClient os, String server_id) throws InterruptedException
	{
		Status status = os.compute().servers().get(server_id).getStatus();
		System.out.print(status);
		if (!status.toString().contentEquals("ACTIVE"))
		{
			os.compute().servers().action(server_id, Action.SUSPEND);
		}
		TimeUnit.SECONDS.sleep(10);		
	...
	private static String GetMethod(String strUrl, String tokenId) throws IOException
	{	
	...
	private static String PostMethod(String strUrl, JSONObject returnedJObject, String tokenId) throws IOException
	{
	StringBuffer strResponse = new StringBuffer(); 	 
	 try
	 {
	...
	public static String GetToken(JSONObject credentials, JSONObject endpointsAPI) throws IOException, org.json.simple.parser.ParseException 
	{
		String tokenIdS = "";
		try
		{			
			String identityService = (String) endpointsAPI.get("os-identity-api") + "/tokens";
	...
	public static String GetImageId(JSONObject credentials, String token, String imgName) throws IOException
	{
		JSONObject auth =  (JSONObject) credentials.get("auth");
		String tenantId = (String) auth.get("tenantId");
	...
	public static String GetInstanceType(JSONObject credentials, String token, String minDisk, String minRAM) throws IOException
	{
		JSONObject auth =  (JSONObject) credentials.get("auth");
		String tenantId = (String) auth.get("tenantId");
		String url = "http://129.69.209.127:8774/v2/"+tenantId+"/flavors?minDisk="+minDisk+"&minRam="+minRAM;
		String instanceType = "";
